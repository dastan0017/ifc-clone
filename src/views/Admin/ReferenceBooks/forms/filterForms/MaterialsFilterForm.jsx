...
export const MaterialsFilterForm = ({ data, onSubmit, hideWindow, resetFilter, initFilterValue }) => {
  const intl = useIntl()

  const validationSchema = Yup.object().shape({
    materialTypeId: Yup.number(),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="form form_container">
      <Row className="window_body">
        <label htmlFor="materialTypeId">{intl.formatMessage({ id: 'reference_books.material_type' })}</label>
        <select name="materialTypeId" aria-label="Default select example" ref={register} defaultValue={-1}>
          <option value={-1} key={-1}>
            {intl.formatMessage({ id: 'global.not_specified' })}
          </option>
          {data?.materialTypesList?.map(el => (
