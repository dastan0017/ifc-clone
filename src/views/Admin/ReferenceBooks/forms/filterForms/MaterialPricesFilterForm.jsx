...
export const MaterialPricesFilterForm = ({ data, onSubmit, hideWindow, resetFilter, initFilterValue }) => {
  const intl = useIntl()
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="form form_container">
      <Row className="window_body">
        ...
        <label htmlFor="supplierId">{intl.formatMessage({ id: 'reference_books.supplier' })}</label>
        <select name="supplierId" aria-label="Default select example" ref={register} defaultValue={-1}>
          <option value={-1} key={-1}>
            {intl.formatMessage({ id: 'global.not_specified' })}
          </option>
          {data?.suppliersList?.map(el => (
            <option value={el.id} key={el.id}>
              {el.name}
            </option>
          ))}
...
MaterialPricesFilterForm.propTypes = {
...
export default MaterialPricesFilterForm
